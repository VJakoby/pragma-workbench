...
function exportTimeline(overrideSessionId) {
  ...
  const targetLabel = t => t ? (t.ip || t.domain || t.label || 'target') : null;
  ...
  if (tlTargetFilter) {
    unified = unified.filter(item => item.kind === 'event' || item.data.target_id === tlTargetFilter);
  }
  ...
  unified.forEach(item => {
    ...
    if (prevTs && (item.ts - prevTs) > idleMs) {
    ...
    if (item.kind === 'note') {
      ...
      md += `**Type:** ${meta.label}`;
      if (tgt) md += `  |  **Target:** ${targetLabel(tgt)}`;
      md += '  \n\n';
