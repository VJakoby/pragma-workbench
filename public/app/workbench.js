...
function openSessionOverviewLoot() {
  if (typeof switchSvcTab === 'function') switchSvcTab('loot');
  if (!document.getElementById('svcPopover')?.classList.contains('open') && typeof toggleSvcPopover === 'function') {
    toggleSvcPopover();
  }
}

function openSessionOverviewPorts() {
  if (typeof switchSvcTab === 'function') switchSvcTab('ports');
  if (!document.getElementById('svcPopover')?.classList.contains('open') && typeof toggleSvcPopover === 'function') {
    toggleSvcPopover();
  }
}

function openSessionOverviewTodos() {
  if (!document.getElementById('todoPopover')?.classList.contains('open') && typeof toggleTodoPopover === 'function') {
    toggleTodoPopover();
  }
}

function openSessionOverviewUnassigned() {
...
function renderSessionOverviewBar() {
  ...
  const latestNoteMeta = latestNote && typeof getNoteTypeMeta === 'function' ? getNoteTypeMeta(latestNote.type) : null;
  ...
  const cards = isFreshSession
    ? [
        ...
        { key: 'Open TODOs', value: 'Add first task', action: "openSessionOverviewTodos()", clickable: typeof toggleTodoPopover === 'function', cardClass: 'card-todos', tooltip: 'Open session TODOs' },
    ...
    : [
        ...
        { key: 'Latest Note', value: latestLabel, action: latestNote ? `openNote('${latestNote.id}')` : '', clickable: !!latestNote && typeof openNote === 'function', cardClass: 'card-neutral', tooltip: latestLabel },
        { key: 'Open TODOs', value: `${openTodos} open`, action: "openSessionOverviewTodos()", clickable: typeof toggleTodoPopover === 'function', cardClass: openTodos > 0 ? 'card-todos' : 'card-neutral', tooltip: `${openTodos} open TODO item${openTodos === 1 ? '' : 's'}` },
        { key: 'Credentials', value: `${credentialCount} found`, action: "openSessionOverviewLoot()", clickable: typeof toggleSvcPopover === 'function' && typeof switchSvcTab === 'function', cardClass: credentialCount > 0 ? 'card-credentials' : 'card-neutral', tooltip: `${credentialCount} cleartext/hash credential entr${credentialCount === 1 ? 'y' : 'ies'}` },
        { key: 'Ports', value: `${serviceCount} service${serviceCount === 1 ? '' : 's'}`, action: "openSessionOverviewPorts()", clickable: typeof toggleSvcPopover === 'function' && typeof switchSvcTab === 'function', cardClass: serviceCount > 0 ? 'card-ports' : 'card-neutral', tooltip: `${serviceCount} logged port/service entr${serviceCount === 1 ? 'y' : 'ies'}` },
        unassignedCount > 0
