    ...
    <div class="session-list-item${s.id === activeSessionId ? ' active-session' : ''}${status === 'complete' ? ' status-complete' : ''}" onclick="switchSession('${s.id}')">
      ...
      <div class="session-list-item-bottom" onclick="event.stopPropagation()">
        <div class="session-item-actions">
          ...
          <button class="session-item-export-btn" onclick="exportNotesMarkdown('${s.id}')" title="Generate a markdown summary from session notes and logs">${ICONS.download} Generate Markdown Summary</button>
        </div>
      </div>
    </div>`;
    }).join('');
